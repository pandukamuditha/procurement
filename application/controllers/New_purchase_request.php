<?php
class New_purchase_request extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('PurchaseRequest_model');
				...
				// elseif((!($session_data['role']->role_id=='depclerk'))||(!($session_data['role']->role_id=='hod'))){
				// 	redirect('dashboard');
				// }
    }
    ...
    public function prDetails()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect('New_purchase_request/assetsDetails');
        }
    }
    ...
    public function addAssets()
    {
        ...
        if ($this->input->post('send')!=null) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                redirect('new_purchase_request/createPR');
            }
        } elseif ($this->input->post('save_as_draft')!=null) {
            $this->PurchaseRequest_model->createPurchaseRequest($data);
            redirect('Purchase/saveDraft');
            //$this->load->view("purchase/saved_draft");
        } elseif ($this->input->post('add_another')!=null) {
            $this->PurchaseRequest_model->addItem($data);
